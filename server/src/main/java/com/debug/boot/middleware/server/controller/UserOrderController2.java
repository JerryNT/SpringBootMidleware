...
@Controller
@RequestMapping("v2")
public class UserOrderController2 extends AbstractController{
    ...
    public BaseResponse add(@RequestBody @Validated UserOrderDto dto, BindingResult result){
        String checkReuslt = ValidatorUtil.checkResult(result);
        if (StringUtils.isNotBlank(checkReuslt)) {
            return new BaseResponse(StatusCode.InvalidParams.getCode(), checkReuslt);
        }
        BaseResponse response=new BaseResponse(StatusCode.Success);
        try {
